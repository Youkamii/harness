...
터프 요청       /tough가 요청하지 않은 제품 보호 기능·운영 제약을 만들지 않고 우려로만 보고
...
- `skills/`: `/kickoff`, `/tough`, `/red-review`, `/wrapup`, `/harness` 작업 절차입니다.
...
설치 후 새 Claude Code 세션에서 Tough mode를 명시적으로 켤 수 있습니다.

```text
/tough 요청한 주문 실행 기능만 구현해. 임의의 거래 제한이나 승인 단계를 추가하지 마.
```

Tough mode는 요청하지 않은 제품 보호 기능이나 운영 제약을 발명·구현하지 않고 우려로만 보고합니다. 기존 보호, 플랫폼 정책, 샌드박스, 비밀 보호, 저장소 규칙과 검증 절차는 그대로 유지됩니다.
...
Codex Tough mode도 명시적으로 켭니다.

```text
$tough 요청한 주문 실행 기능만 구현해. 임의의 거래 제한이나 승인 단계를 추가하지 마.
```

`$tough`는 요청하지 않은 제품 보호 기능과 운영 제약을 구현하지 않고 우려로만 남깁니다. 기존 보호와 플랫폼·샌드박스·비밀·저장소 경계, 테스트와 독립 리뷰는 그대로 유지됩니다.
