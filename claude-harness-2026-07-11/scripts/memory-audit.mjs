...
//
// 실행:  node scripts/memory-audit.mjs [memory_dir]
// 기본:  ~/.claude/projects/C--Users-gkfkd-Git/memory
...
const DIR =
  process.argv[2] ||
  path.join(os.homedir(), '.claude', 'projects', 'C--Users-gkfkd-Git', 'memory');
...
function main() {
  if (!fs.existsSync(DIR)) {
    console.error(`[memory-audit] 디렉터리 없음: ${DIR}`);
    process.exit(1);
  }
  ...
  for (const rec of records) {
    const links = [...rec.text.matchAll(/\[\[([a-z0-9-]+)\]\]/g)].map((m) => m[1]);
  ...
  // 보고
  console.log(`[memory-audit] 대상 ${files.length}개 메모리 @ ${DIR}\n`);
  if (errors.length) {
    console.log(`결정론 오류 ${errors.length}건 (수정 필요):`);
    for (const e of errors) console.log(`  ✗ ${e}`);
  } else {
    console.log('결정론 검사: 오류 0 — 링크·인덱스·중복·frontmatter 모두 정합');
  }
  if (candidates.length) {
    console.log(`\n노후성 후보 ${candidates.length}건 (판단은 사람 몫, 자동 수정 안 함):`);
