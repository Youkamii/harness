...
const SCENARIOS = [
  ...
  ['터프 모드 활성 조건', /\/tough <요청>.*터프 모드로 구현해.*use tough mode/s, '명시적 활성화 지시 때만 활성화'],
  ['터프 모드 비활성 조건', /단순 질문.*부정.*인용·코드·예시.*켜지 않는다/s, '질문·부정·인용에는 비활성'],
  ['터프 모드에서 요청·계약 밖 보호 장치', /요청하지 않았고 기존 제품 계약에도 없는 보안·안전·보호 기능.*발명·구현하지 않는다/s, '범위 밖 발명·구현 금지'],
  ['터프 모드의 우려 처리', /상위 경계에 해당하지 않는 한.*질문으로 작업을 막지 말고.*우려.*로만/s, '상위 경계 밖 우려로만 보고'],
  ['터프 모드의 실제 경계 충돌', /상위 경계나 기존 계약.*실제로 충돌하면.*충돌을 보고/s, '조용한 우회 금지'],
  ['터프 모드에서 기존 보호', /이미 있는 보호 기능.*삭제·완화·우회하지 않는다/s, '기존 보호 유지'],
  ['터프 모드의 상위 경계', /플랫폼 정책.*샌드박스.*override하지 않는다/s, '플랫폼·샌드박스 유지'],
  ['터프 모드에서 요청·계약에 든 보호 기능', /요청했거나 기존 제품 계약에 포함된 보호 기능.*유지·구현한다/s, '요청·기존 계약 유지'],
  ['터프 모드의 세부 우선순위', /보안 지적의 절제.*더 구체적인 규칙.*스킬 절차와 자율 판단보다 우선/s, '활성 시 구체 규칙 우선'],
];
