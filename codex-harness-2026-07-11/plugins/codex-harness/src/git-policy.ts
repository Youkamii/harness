...
export async function assertSafeGitConfiguration(
  repoRoot: string,
  ...
  if (!paths) {
    ...
    if (tracked.exitCode !== 0) throw new Error(`unable to inspect tracked paths: ${tracked.stderr.trim()}`);
  ...
  for (let index = 0; index + 2 < fields.length; index += 3) {
    ...
    if (value && !["unspecified", "unset", "set"].includes(value)) active.add(value);
  }
  for (const name of active) {
    if (!/^[A-Za-z0-9_.-]+$/.test(name)) throw new Error("invalid Git filter name");
