...
export async function setTaskStatus(
  ...
  return await store.update(
    ...
    (state) => {
      ...
      if (to === "complete") delete task.lastFailure;
...
export async function clearTaskWorktree(
  store: RunStore,
  runId: string,
  taskId: string,
): Promise<RunState> {
...
export async function recordRunBase(
  store: RunStore,
  runId: string,
  baseSha: string,
): Promise<RunState> {
  if (!/^[0-9a-f]{40,64}$/i.test(baseSha)) throw new Error("invalid run base SHA");
  return await store.update(
    runId,
    "run.base.recorded",
    (state) => {
      if (state.baseSha && state.baseSha !== baseSha) throw new Error("run base SHA is immutable");
...
export async function reopenTaskForRemediation(
  store: RunStore,
  runId: string,
  taskId: string,
  reason: string,
  ...
  return await store.update(
    runId,
    "task.remediation.prepared",
    (state) => {
      const task = requireTask(state, taskId);
      if ((task.status === "ready" || task.status === "pending") && task.lastFailure) return state;
      if (task.status !== "complete") throw new Error(`task ${taskId} is not complete`);
      const complete = new Set(
        state.tasks.filter((candidate) => candidate.status === "complete").map((candidate) => candidate.id),
      );
...
export async function beginRemediation(
  store: RunStore,
  runId: string,
  taskId: string,
  reason: string,
  ...
  return await store.update(
    runId,
    "run.remediation.started",
    (state) => {
      if (state.status === "remediating" && state.remediation) return state;
...
export async function completeRemediation(store: RunStore, runId: string): Promise<RunState> {
  return await store.update(
    runId,
    "run.remediation.completed",
    (state) => {
      if (state.status !== "remediating") throw new Error("run is not remediating");
...
export async function clearIntegration(store: RunStore, runId: string): Promise<RunState> {
  return await store.update(
    runId,
    "run.integration.cleared",
    (state) => {
