...
interface FingerprintTraversal {
  depth: number;
  visited: Set<string>;
}
...
async function fingerprintRepository(
  canonicalRoot: string,
  traversal: FingerprintTraversal,
): Promise<string> {
  ...
  for (const relativePath of files) {
    ...
    if (gitlink) {
      ...
      hashes.push(
        `${relativePath}:gitlink:index=${gitlink}:working=${working.exitCode === 0 ? working.stdout.trim() : "unavailable"}:content=${nestedFingerprint}`,
      );
      continue;
    }
    try {
...
async function trackedGitlinks(repoRoot: string): Promise<Map<string, string>> {
  ...
  for (const record of result.stdout.split("\u0000").filter(Boolean)) {
    ...
    if (match?.[1] === "160000" && match[2] && match[3]) gitlinks.set(match[3], match[2]);
  }
  return gitlinks;
}
