...
function topologicalTasks(tasks) {
    ...
    while (pending.size > 0) {
        const ready = [...pending.values()].filter((task) => task.dependencies.every((dependency) => emitted.has(dependency)));
        if (ready.length === 0)
            throw new Error("task graph cannot be topologically ordered");
...
async function taskCommits(repoRoot, task) {
    ...
    if (commits.length === 0)
        throw new Error(`task ${task.id} has no commits after its base`);
    return commits;
}
async function effectForCommit(repoRoot, commit, taskId) {
    ...
    if (!effectId)
        throw new Error(`commit ${commit} lacks a Harness-Effect trailer`);
    return effectId;
}
async function commitForEffect(worktreePath, baseSha, effectId) {
    const result = await runChecked({
        executable: "git",
        args: [
            "log",
