        ...
        while (true) {
            ...
            if (state.status === "executing") {
                ...
                if (incomplete.length > 0) {
                    ...
                    if (!actionable)
                        throw new Error("all incomplete tasks are waiting on unsatisfied dependencies");
            ...
            if (state.status === "reviewing") {
                ...
                if (!commitSha)
                    throw new Error("integration SHA is missing");
                for (const task of state.tasks) {
...
async function advanceTask(store, runId, observed) {
    ...
    if (task.status !== "complete")
        throw new Error(`task ${task.id} cannot advance from ${task.status}`);
}
async function failAndRetry(store, runId, taskId, error) {
    let state = await store.load(runId);
...
async function retryOrExhaust(store, runId, task, reason) {
    if (task.attempts < MAX_TASK_ATTEMPTS) {
        await resetTaskForRetry(store, runId, task.id, redactSecrets(reason));
        return;
    }
    if (task.status === "failed")
        await setTaskStatus(store, runId, task.id, "blocked");
    await blockRun(store, runId, `Task ${task.id} exhausted ${MAX_TASK_ATTEMPTS} attempts: ${redactSecrets(reason)}`);
}
function canFail(status) {
...
function requireIntegrationWorktree(state) {
    if (!state.integrationWorktreePath)
        throw new Error("integration worktree is missing");
