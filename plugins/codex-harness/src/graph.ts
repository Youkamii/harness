...
export function validatePlan(tasks: PlannedTask[]): void {
  ...
  if (tasks.length > 64) throw new Error("plan exceeds the 64 task limit");

  const ids = new Set<string>();
  for (const task of tasks) {
    if (!taskIdPattern.test(task.id)) throw new Error(`invalid task id: ${task.id}`);
    ...
    for (const check of task.checks) {
      ...
      if (check.cwd) assertRelativeSafePath(check.cwd, `task ${task.id} check cwd`);
    }
  }
...
function assertOwnedPathsDoNotOverlap(tasks: PlannedTask[]): void {
...
function assertDepth(tasks: PlannedTask[]): void {
  const byId = new Map(tasks.map((task) => [task.id, task]));
  ...
  for (const task of tasks) {
    if (depth(task.id) > 16) throw new Error("task dependency depth exceeds 16");
  }
}

function assertRelativeSafePath(value: string, label: string): void {
...
export function materializeTasks(tasks: PlannedTask[]): HarnessTask[] {
  validatePlan(tasks);
...
export function refreshReadyTasks(tasks: HarnessTask[]): HarnessTask[] {
