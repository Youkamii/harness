...
1. 훅 등록 확인: `~/.claude/settings.json`의 hooks.PreToolUse에 guard.js·secrets-guard.mjs 항목이 있는지.
2. 가드 회귀 테스트: `node <레포>/hooks/guard.test.js` — ALL PASS 확인.
3. 파일 존재: `~/.claude/skills/{kickoff,red-review,wrapup,harness}`, `~/.claude/agents/{red-reviewer,verifier}.md`.
4. 레포-로컬 diff: 레포와 배포본이 다르면 보고하고 재설치를 제안.
